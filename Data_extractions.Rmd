...
Imagine, you quickly got well and instead of reproducing decided to carry on the Terminal Investment Meta-analysis Project.   
After a few weeks you fished searching and screening literature for the relevant papers. You now have a collection of around 30 included full-text papers (it could be 300 as well), stored neatly as pdf files and supplementary information, in whatever format available. Its time to start extracting the data (ideally, you should have planned this in detail while writing up your protocol/preregistration, but in this exercise we assume you did not).   
...
Before you extract actual data for the meta-analysis you need some sort of system to manage data extractions and store the data (we assume by now you already have a system for organising and storing your references and papers, so we don't cover this). For data extractions, many people just stick to Excel spreadsheets, or any other spreadsheets or extraction forms that can be converted into spreadsheets (e.g. Google Forms). Some researchers even build elaborated relational databases (there is a high initial investment cost but many benefits later on). Whatever you chose, you need a platform where you can freely add and edit columns and be able to store different types of data, so rigid systems purpose-built for medical trial data are not recommended. In this exercise we will stick to the simplest, but flexible platform - Excel spreadsheet.    
...
**QUESTION 4:**   
Whats wrong with this table representing a fragment of an extraction spreadsheet?        
...
**QUESTION 9:**   
What is meta-data in meta-analysis and what it is used for?   


**Note:** In your actuall extraction sheet/form, try to group related columns together. You can also color-code them in Excel or put on the same tab/section in a visual database interface, if used. Your extraction form (spreadsheet) should be tested on several papers (pilot extractions) to make sure its easy to use and captures necessary information.  
...
**TASK 1**:  
We will practice data extraction on few of the Terminal investment papers. We will use R package *metaDigitise*. 
Detailed package description and tutorial are available at: https://cran.r-project.org/web/packages/metaDigitise/vignettes/metaDigitise.html
...
Here is the package installation code:
...
You will need to open the pdf files and take the snapshots of the relevant figures. Make sure you capture the axes and axes labels. Also, enlarge these figures to fill the screen before taking the snapshot to get higher resolution image.  

Name the files as following (this makes it easy to keep track of the figures being digitised):     
001_Weil_2006_Fig2a.png    
and    
002_Sadd_2006_Fig2a.png   

We need to place them in a new folder - we called it metaDigitise_figures (read more about organising your extraction directories in the "Setting up directory structures " section of https://cran.r-project.org/web/packages/metaDigitise/vignettes/metaDigitise.html).   
...
Tell metaDigitise() what you would like to do by selecting option  (Process new images - enter "1").   
...
First plot should be loaded at this stage. We do not need to rotate the image, so enter "c".   

Now we can store the information about y-variable type - enter "testes volume (mm3)".   

You get asked for the data needed to calibrate y-axis. Click on 200 and 900, then enter these values as prompted and complete calibration.   

Its good to enter sample sizes at this point. Since there is no such info in the plot or under the plot, search the full text for the sample sizes (hint: look in "Statistical anlaysis"). Enter identifier for the control group ("saline") and the sample size.    


Since we have multiple data points for each group of animals, we have to decide whether extract all of them or just one (or some subset) - your decision should be biologically-informed and ideally a priori. For this exercise we will extract just the last one (at 6 weeks). Click on the outer edge of the error bar of the last black point, and then click the middle of the black dot. If it went well you can add the treatment group "LPS" in a similar manner. If something went wrong you can delete and repeat the extraction.  
...
**QUESTION 13:**   
Why we need to change our data frame (and how)?      


**TASK 2**:
Since we have only 2 data points per file, its easy to reshape the data frame in R.   
...
Now the columns appended with .x contain control groups data values, and these with .y are for the treatment groups (you can rename the columns to make it easier to see).   

**TASK 3**:  
We will now use *escalc* function from *metafor* package to calculate effect sizes. The function allows to calculate various effect sizes or outcome measures (and the corresponding sampling variances) that are commonly used in meta-analyses, including two effect sizes are relevant to our data and question:   
...
Open the above webpage. Main menu has an expandable list of types of effect sizes, and statistical tests you can use to calculate these effect sizes (only d, r, OR and RR).   
...
Use online ES calculator sheet selecting "Standarized Mean difference (d)"" and "F-test, 2 groups" for to calculate d from one-way ANOVA (you will need F-value and sample sizes of treatment and control groups).   
...
**TASK 5**:  
Using web-based ES calculator, estimate a few other effect sizes using test (inferential) statistics reported in Sadd2006.
...
**QUESTION 16:**    
Is the sign of effect size important? Imagine we use d, which represents mean_treatment - mean_control group trait values. How we would interpret d = 0.5 and how we would interpret d = -0.5?


**QUESTION 17:**    
If we have different traits in the data set (lets say number of eggs laid and probability of next abandonment) what is the problem with the  signs of effect sizes for these traits? What should we do about this?   
...
**QUESTION 18:**   
As you hopefully remember, PRISMA checklist is a document that helps making sure that you reported the key information how a systematic review (including meta-analysis) was done.    
...
**QUESTION 19:**   
Although not mentioned in the original PRISMA checklist, it is important to share the collected data, meta-data and analysis code? Think of east at least 3 reasons for doing so.
